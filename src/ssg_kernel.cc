...
void ssg_kernel::compile_program()
{
...
#ifdef BINARY
    {
       FILE *fp = fopen("./kernel/kernel_binary", "rb");
       if (fp == NULL)
       {
           printf("fail to load binary\n");
       }
    ...
    if(err !=0)
    {
        ...
        for (cl_uint i = 0; i < numDevices; i++)
        {
            ...
            if (devices[i] == device)
            {
                FILE *fp = fopen("./kernel/kernel_binary", "wb");
...
#ifdef debug
    FILE * fp_col_gpu = fopen("./to_mnist/col_gpu_n2.txt","w");
    
    for(int iter=0;iter<col_height * col_step;iter++)
        fprintf(fp_col_gpu,"%f\n",im2col_out[iter]);
...
{
    FILE * fp_tmp = fopen("./to_mnist/col_cpu_n2.txt","w");
...
void ssg_kernel::gemm::gemm_gpu()
{
    ...
    int glob_h_step = col_h;
    int glob_w_step = 1;
#elif KERNEL ==3
	const size_t local[2] = { TS, TS / WPT };
	const size_t global[2] = { (size_t)M, (size_t)(N / WPT) }i;
#elif KERNEL == 6 
    ...
    err = clSetKernelArg(kernel1,4,sizeof(int),(void*)&glob_h_step);
    err = clSetKernelArg(kernel1,5,sizeof(int),(void*)&glob_w_step);
    err = clSetKernelArg(kernel1,6,sizeof(cl_mem),(void*)&parent.buf_col);
    err = clSetKernelArg(kernel1,7,sizeof(cl_mem),(void*)&parent.buf_weight);
    err = clSetKernelArg(kernel1,8,sizeof(cl_mem),(void*)&parent.buf_bias);
    err = clSetKernelArg(kernel1,9,sizeof(cl_mem),(void*)&parent.buf_gemm);
    err = clSetKernelArg(kernel1,10,sizeof(cl_mem),(void*)&bufSubA);
    err = clSetKernelArg(kernel1,11,sizeof(cl_mem),(void*)&bufSubB);
    parent.checkError(err,__LINE__);
    ...
    FILE * fp_subA = fopen("./to_mnist/subA.txt","w");
    FILE * fp_subB = fopen("./to_mnist/subB.txt","w");
    FILE * fp_gemm_gpu = fopen("./to_mnist/gemm_gpu_n2.txt","w");

    for(int h=0;h<col_height;h++){
        for(int w=0;w<col_step;w++){
            fprintf(fp_subA,"%f",subA[h*col_step+w]);
        }
        fprintf(fp_subA,"\n");
    }
    ...
    for(int h=0;h<weight_h;h++){
        for(int w=0;w<weight_w;w++){
            fprintf(fp_subB,"%f",subB[h*weight_w+w]);
        }
        fprintf(fp_subB,"\n");
    }
     ...
     for(int h=0;h<col_height;h++){
        for(int w=0;w<weight_w;w++){
            fprintf(fp_gemm_gpu,"%f\n",gemm_out[h*weight_w+w]);
        }               
    }
...
#ifdef debug
    FILE * fp_tmp = fopen("./to_mnist/gemm_cpu_n2.txt","w");
...
void ssg_kernel::maxpool::maxpool_cpu()
{
    ...
    FILE * fp_tmp = fopen("./to_mnist/maxpool_cpu_n2.txt","w");
...
#ifdef debug
    FILE * fp_maxpool_gpu = fopen("./to_mnist/maxpool_gpu_n2.txt","w");

    for(int h=0;h<col_height;h++){
        for(int w=0;w<img_c;w++){
            fprintf(fp_maxpool_gpu,"./to_mnist/%f\n",pool_out[h*img_c+w]);
        }
    }
